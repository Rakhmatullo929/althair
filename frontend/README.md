...
│   └── client/               Auth, onboarding, and settings portal on :3001
├── packages/
...
and versioned AI Context pages. RU, UZ, and EN dictionaries are complete. Real provider activation,
OpenAI execution, CRM, booking, billing, and Super Admin are intentionally absent.
...
Client E2E creates an isolated SQLite database in `/tmp`, starts a deterministic local backend,
and runs without any provider or production email dependency. Screenshots are written below
`artifacts/screenshots/client/` and remain ignored by Git.
